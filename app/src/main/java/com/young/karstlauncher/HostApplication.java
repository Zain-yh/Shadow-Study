...
import com.young.karstlauncher.manager.Shadow;
...
public class HostApplication extends Application {
    ...
    private PluginManager mSingletonPluginManager;
    Map<String, PluginManager> managerMap = new HashMap<>();
    ...
    public void onCreate() {
        ...
        if (isProcess(this, "karstlauncher")) {
...
//        HostUiLayerProvider.init(this);
    ...
    public void loadPluginManager(File apk) {
        if (mSingletonPluginManager == null) {
            mSingletonPluginManager = Shadow.getPluginManager(apk);
        }
    }

    public PluginManager getmSingletonPluginManager() {
        return mSingletonPluginManager;
    }

    public PluginManager getPluginManager(String key, File apk) {
        PluginManager pluginManager = managerMap.get(key);
        if (pluginManager == null) {
            pluginManager = Shadow.getPluginManager(apk);
            managerMap.put(key, pluginManager);
        }
